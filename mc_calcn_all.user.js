...
//　配布URL
var se_rack = "<b><内蔵版>ＭＣ☆あくしず 領地敵兵算出機 by gozen ";
var url_rack = "";
var com_rack = "出現目安です！ 初心者から変態まで御用達の通期版（とりあえず5期までは大丈夫！）";
var url_rack2 = "https://github.com/5zen";
...
//　出撃先のタイル数格納変数（平地、鉄鋼山、油田、ボーキサイト鉱山、田畑、荒地）の順。
var tile = new Array(0,0,0,0,0,0);

//　出撃先のパネル構成ごとの兵数計算パラメータ
//　[新兵、船、戦、飛、駆、重戦、重飛]の順に格納。
//
// 鯖判定用
var sv_seazon = -1;
...
var point1ki = {
	...
	//1期
	//鉄油ボ田空	   歩	船		戦	   飛		 駆	   重戦	   重飛
	"000116"	:	[	5,	 0,		 0,		0,		  0,	  0,	  0,	"☆1※"							],	//☆1※
	"100016"	:	[	5,	 0,		 0,		0,		  0,	  0,	  0,	"☆1鉄鋼山"						],	//☆1鉄鋼山
	"010016"	:	[	5,	 0,		 0,		0,		  0,	  0,	  0,	"☆1油田"						],	//☆1油田
	"001016"	:	[	5,	 0,		 0,		0,		  0,	  0,	  0,	"☆1ボーキサイト鉱山"			],	//☆1ボーキサイト鉱山
...
var def = {
	//			   歩,   船,   戦,   飛,  駆, 重戦, 重飛
	"ken"	:	[  15,   52,   50,   54, 208,  200,  216],	//新兵防御
	"yari"	:	[  10,   58,   40,   28, 145,  100,   70],	//軽戦車防御
	"yumi"	:	[  10,   42,   25,   60, 105,   63,  150],	//水雷艇防御
	"ki"	:	[  10,   26,   55,   44,  65,  137,  110]	//軽戦闘機防御
};
...
(function(){
		...
		for(i=0;i<serverTable.length;i++){
			var BRO3_TOBATSU_SV	= 'mc_tobatsu_sv' + i;
			var BRO3_TOBATSU_SEASON	= 'mc_tobatsu_season' + i;
		...
		if(season>0){
			...
			//　出撃先タイルパターン取得のためのHTTP要求　starに★数、tile[]に各タイル数を格納。
			...
			msg = msg + "<b>　＋　賊討伐シミュレータ by gozen. ver"+gozen_ver+"</b><br>\n";
			msg = msg + "</td></tr></table>\n";
...
function getFieldType(x){
	...
	if(list==undefined){
		...
		msg = msg + "<tr><td align=left>\n";
		msg = msg + se_rack + ver_rack + "<br>\n";
		msg = msg + "<b>　＋　賊討伐シミュレータ by kino. ver"+gozen_ver+"</b><br>\n";
		msg = msg + "</td></tr></table>\n";
...
console.log("power@2:" + power);
		...
		msg = msg + "出撃元：<a href='http://"+location.hostname+"/map.php?x=" + bcordx +"&y=" + bcordy + "'>(" + bcordx +"," + bcordy + ")</a><br>\n";
		...
		msg = msg + "</font>\n";
		msg = msg + "歩兵科："		+ red_s + String(kenk)	+ red_f + "～" + gre_s + String(kenk2)	+ red_f + "<br>\n";
		msg = msg + "軽戦車科："	+ red_s + String(yarik)	+ red_f + "～" + gre_s + String(yarik2)	+ red_f + "<br>\n";
		msg = msg + "水雷艇科："	+ red_s + String(yumik)	+ red_f + "～" + gre_s + String(yumik2)	+ red_f + "<br>\n";
		msg = msg + "軽戦闘機科："	+ red_s + String(kik)	+ red_f + "～" + gre_s + String(kik2)	+ red_f + "<br>\n";
		msg = msg + "<font color=white>The無強化兵士に単純換算<br>\n";
		msg = msg + "歩:"+Math.ceil(kenk/atk["ken"]) 	+ "～" + Math.ceil(kenk2/atk["ken"])	+	"<br>\n";
		msg = msg + "戦:"+Math.ceil(yarik/atk["yari"])	+ "～" + Math.ceil(yarik2/atk["yari"])	+	"(重戦車"	+	Math.ceil(yarik/atk["hoko"])	+ "～" + Math.ceil(yarik2/atk["hoko"])	+	"）<br>\n";
		msg = msg + "船:"+Math.ceil(yumik/atk["yumi"])	+ "～" + Math.ceil(yumik2/atk["yumi"])	+	"(駆逐艦"	+	Math.ceil(yumik/atk["do"])		+ "～" + Math.ceil(yumik2/atk["do"])	+	"）<br>\n";
		msg = msg + "飛:"+Math.ceil(kik/atk["ki"])		+ "～" + Math.ceil(kik2/atk["ki"])		+	"(重戦闘機"	+	Math.ceil(kik/atk["kono"])		+ "～" + Math.ceil(kik2/atk["kono"])	+	"）</font></span><br>\n";

		msg = msg + "</td><td align=center style='text-align:center;background:#FFC;color:#C60;'>\n";

		msg = msg + "<br><b><font size='+1'>賊討伐シミュレータ</font> ver "+gozen_ver+"</b><br><br>\n";
		msg = msg + "<form>\n";

		msg = msg + "<div align='right'><input type='button' id='get_kajiba' value='武装研究所・課金状況取得'></div>\n";
		...
		msg = msg + "<td>武将兵科:<select id='b_type'><option value=0>歩<option value=1>戦<option value=2>船<option value=3>飛</select>\n";
		msg = msg + "</tr><tr>\n";
		msg = msg + "<td>スキル：</td><td colspan=3 align=right><select id='skill'><option value=0>武将<option value=1>新兵・歩兵兵科の武将<option value=2>軽戦車・重戦車・軽戦車兵科の武将<option value=3>水雷艇・駆逐艦・水雷艇兵科の武将<option value=4>軽戦闘機・重戦闘機・軽戦闘機兵科の武将<option value=5>全ての兵士・武将<option value=6>重戦闘機・重戦車・駆逐艦</select><br>の攻撃力が<input type='text' value=0 id='skill_rate' size=4 style='text-align:right;' />%上昇する</td>\n"
		msg = msg + "</tr><tr>\n";
		msg = msg + "</tr><tr>\n";
	...
	function get_kaji_lv(){
		...
		for (var i = 0; i < panels2.snapshotLength; i++) {
			...
			if(panels3.indexOf("武装研究所") != -1) {
				...
				if (sysmes.indexOf("現在は下記兵種の武装を強化しています") != -1) num += 1;
				...
				for (var i = 1; i < trs.snapshotLength; i++) {
					...
					if(xxx_tr.indexOf("軽戦闘機") != -1 && xxx_tr.indexOf("重戦闘機") == -1 && xxx_tr.indexOf("特殊部隊") == -1) {
						var tr_ki = i+1;
	...
	function sol_index(){
		...
		else if(soltype == "軽戦車") s_index = 1;
		else if(soltype == "水雷艇") s_index = 2;
		else if(soltype == "軽戦闘機") s_index = 3;
